...
from utils.contracts import contract_skydrome
from config import TOKEN_SKYDROME
...
class Skydrome:
    ...
    async def swap_eth_to_token(self, amount: TokenAmount, contract_token: str, retry = 0, slippage = 1):
        ...
        try:
            contract = self.client.w3.eth.contract(address=contract_skydrome, abi=Skydrome.abi)
            ...
            tx = self.client.send_transaction(
                ...
                data=contract.encodeABI('swapExactETHForTokens', args=(
                    min_to_amount.Wei,
                    [('0x5300000000000000000000000000000000000004', contract_token, False)],
                    self.client.address,
    ...
    async def swap_token_to_eth(self, contract_token: str, retry = 0, amount: Optional[TokenAmount] = None, slippage = 0.1):
        ...
        print(f"{self.client.address} | Skydrome Swap | {format(float(value.Ether), '.5f')} {self.client.name(contract_token)} --> ETH...")
        logger.info(f"{self.client.address} | Skydrome Swap | {format(float(value.Ether), '.5f')} {self.client.name(contract_token)} --> ETH...")
        try:
            chek = chek_gas_eth(self.maxGas)
            if chek == False:
                return 0
            ...
            tx = self.client.send_transaction(
                ...
                data=contract.encodeABI('swapExactTokensForETH',
                                        args=(
                                            ...
                                            min_to_amount.Wei,
                                            [
                                                (contract_token, "0x5300000000000000000000000000000000000004",  False)
                                            ],
                                            self.client.address,
                                            int(time.time()) + random.randint(600, 1200)
                                        )),
    ...
    async def AllStableSell(self):
        print(f"{self.client.address} | Skydrome AllStableSell...")
        logger.info(f"{self.client.address} | Skydrome AllStableSell...")
        try:
