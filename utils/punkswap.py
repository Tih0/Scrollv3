...
class Punkswap:
    ...
    async def swap_eth_to_token(self, amount: TokenAmount, contract_token: str, retry = 0, slippage = 2):
        print(f"SWAP | PunkSwap | {format(float(amount.Ether), '.6f')} ETH --> {self.client.name(contract_token)}...")
        logger.info(f"SWAP | PunkSwap | {format(float(amount.Ether), '.6f')} ETH --> {self.client.name(contract_token)}...")
        try:
