...
from config import TOKEN_USDC, useProxy
...
class Client:
    ...
    def name(self, contract_address: str):
        return self.w3.eth.contract(address = Web3.to_checksum_address(contract_address), abi= self.abi).functions.name().call()

    def get_allowance(self, token_address: str, spender: str) -> TokenAmount:
        contract = self.w3.eth.contract(address=Web3.to_checksum_address(token_address), abi=self.abi)
        return TokenAmount(
    ...
    def verif_tx(self, tx_hash) -> bool:
        try:
            ...
            if 'status' in data and data['status'] == 1:
                ...
                logger.info(f'{self.address} | transaction was successful: {self.network.explorer}{tx_hash.hex()}')
                return True
            else:
